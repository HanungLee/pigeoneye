											...
											const UINT16* pInfraredBuffer)
{
	...
	//	HRESULT hr1 = m_pCoordinateMapper->MapDepthFrameToColorSpace(nDepthWidth * nDepthHeight, (UINT16*)pDepthBuffer, nDepthWidth * nDepthHeight, m_pColorCoordiantes);
		...
		if (SUCCEEDED(hr2))
		{
			...
			{
				const UINT16* pBuffer = pInfraredBuffer;
				const UINT16* pInfraredBufferEnd = pBuffer + (cDepthWidth * cDepthHeight);
				IplImage* cvRawInfraredImage = cvCreateImage(cvSize(cDepthWidth, cDepthHeight), IPL_DEPTH_16U, 1);
				int pixelCounter = 0;

				while (pBuffer < pInfraredBufferEnd)
				{
					int x = pixelCounter % cDepthWidth;
					int y = pixelCounter / cDepthWidth;
					cvSet2D(cvRawInfraredImage, y, x, cvScalar(*pBuffer >> 3));
					++pixelCounter;
					++pBuffer;
				}
				/*
				std::stringstream RawInfraredImageFilename;
				RawInfraredImageFilename << "./infrared_data/infrared_" << 
					std::setfill('0') << std::setw(5) << m_nFrameSaverCounter << '_' <<
					std::setfill('0') << std::setw(12) << nTime << ".png";
				*/
				{
					cv::Mat cvInfraredImage(cvRawInfraredImage);
					cv::Mat adjMap;
					cv::convertScaleAbs(cvInfraredImage, adjMap, 10000.0 / 1000.0);
